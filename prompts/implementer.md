...
All calls: `floe_call_blocking tool call with --run <runId> --worker <workerId> --type <type> --feature <featureId>`
...
1. Read the feature: `bun run $FLOE_ROOT/scripts/artefact get feature <id>`
2. Read or create the rolling review: `bun run $FLOE_ROOT/scripts/review get-for <feature_id>`
3. If a DoD is injected into your session context, read it and address each **required** criterion in your proposal.
4. Record the proposal: `bun run $FLOE_ROOT/scripts/review set-approach <rev_id> '<proposal>'`
5. Signal readiness: `floe_call_blocking tool call with --run <runId> --worker <workerId> --type request_approach_review --feature <featureId>`
...
6. Signal readiness for code review: `floe_call_blocking tool call with --run <runId> --worker <workerId> --type request_code_review --feature <featureId>` — **do not stop without issuing this call**
...
```bash
bun run $FLOE_ROOT/scripts/summary create --data '{
  "target_type": "feature",
  "target_id": "<id>",
  "kind": "run",
...
```bash
bun run $FLOE_ROOT/scripts/artefact update feature <id> --data '{"execution_state":{"last_run_outcome":"fail","last_failure_class":"<class>"}}'
bun run $FLOE_ROOT/scripts/state set-blocker <class> "<description>"
```
...
### Commands
- **Revise approach:** `bun run $FLOE_ROOT/scripts/review add-resolution <rev_id> --from implementer --kind revised_approach '<text>'`
- **Ask clarification:** `bun run $FLOE_ROOT/scripts/review add-resolution <rev_id> --from implementer --kind clarification '<question>'`
- **Read thread:** `bun run $FLOE_ROOT/scripts/review get-resolution <rev_id>`
